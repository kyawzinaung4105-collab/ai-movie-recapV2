...
export function CustomTranslationWorkflow({ duration, videoUrl, onTranslationApplied }: CustomTranslationWorkflowProps) {
  ...
  const prompt = useMemo(() => `You are a professional football news subtitle writer and Burmese sports-news narrator. Rewrite each numbered English line as concise, natural, conversational Burmese that sounds like a real Myanmar football commentator speaking. Do NOT translate word-for-word or preserve English grammar. Use natural Burmese sentence order, particles, connectors, and football-news expressions. Preserve the story's narrative flow: the opening context, who did what, cause and effect, turning points, emotional tone, important reactions, and the logical connection between one line and the next. Do not shorten by deleting the context that makes the story understandable. Preserve the essential meaning and every important fact: player names, team names, scores, dates, places, and football terms. Remove only repetition, filler words, unnecessary explanations, and awkward literal wording so each line is shorter but still feels like part of the same story. Do not add facts, guess missing information, invent events, or make the narration sound like disconnected headlines. This video is ${durationText} long and the translated lines will be distributed across the full video. The average available time is about ${perLineSeconds.toFixed(1)} seconds per line. Each line MUST be short enough to speak naturally inside its assigned time. The total spoken narration MUST NOT exceed ${durationText}; if a line would run long, rewrite it more compactly while preserving the story flow and key meaning. Do not create extra content just to make the audio longer; use only natural short pauses when needed. Keep each numbered line as one subtitle unit. Every text value must contain a non-empty Burmese translation; never return an empty string. Return ONLY valid JSON in this exact format: {"translations":[{"line":1,"text":"ဇာတ်လမ်းအဆက်အစပ်မပျက်တဲ့ တိုတောင်းပြီး သဘာဝကျတဲ့ မြန်မာစာကြောင်း"},{"line":2,"text":"နောက်ထပ် ဆက်စပ်မှုရှိတဲ့ စာကြောင်း"}]}. You MUST return exactly one object for every line number from 1 to ${englishLines.length}; do not skip, duplicate, reorder, or invent line numbers. Do not add explanations, markdown, or code fences.\n\nNumbered English transcript:\n${numberedTranscript}`, [numberedTranscript, englishLines.length, durationText, perLineSeconds]);
  ...
  const correctionPrompt = useMemo(() => `The previous Burmese subtitle output is incomplete, too long, or has a wrong line mapping. Regenerate the COMPLETE result in concise, natural, conversational Burmese football-news style, not word-for-word Burmese. This video is ${durationText} long, so the narration must fit inside the full video and MUST NOT run beyond the video end. Shorten only repetition, filler, and awkward wording; preserve the story's opening context, cause-and-effect, turning points, reactions, emotional tone, and logical connection between lines. The result must still feel like one complete football story, not disconnected short headlines. Compare the numbered English source with the current output. Restore missing lines, but do not add facts, merge lines, split lines, reorder, invent, or change player names, teams, scores, dates, or places. Each line must be short enough for its assigned subtitle interval. Return ONLY valid JSON in this exact format: {"translations":[{"line":1,"text":"ဇာတ်လမ်းအဆက်အစပ်မပျက်တဲ့ တိုတောင်းပြီး သဘာဝကျတဲ့ စာကြောင်း"},{"line":2,"text":"နောက်ထပ် ဆက်စပ်မှုရှိတဲ့ စာကြောင်း"}]}. Return exactly one non-empty object for EVERY line number from 1 to ${englishLines.length}, in numeric order. Never return a plain string array.\n\nNumbered English source:\n${numberedTranscript}\n\nCurrent incomplete or overlong output:\n${jsonOutput}`, [englishLines.length, numberedTranscript, jsonOutput, durationText]);
